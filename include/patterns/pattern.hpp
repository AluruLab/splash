...
#ifdef USE_MPI
        M2VProcessor(MPI_Comm comm = MPI_COMM_WORLD) {
            MPI_Comm_size(comm, &procs);
            MPI_Comm_rank(comm, &rank);
        }
#else
        M2VProcessor() : procs(1), rank(0) {};
#endif
        M2VProcessor(int const & _procs, int const & _rank) :
...
class MVMV2MProcessor {
    ...
    public:
        ...
        OUT operator()(INM const & input1, INV const & input1_aux, 
