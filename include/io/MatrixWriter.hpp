...
public:
	...
	static bool storeMatrixData(std::string const & fileName, std::vector<std::string> const & row_names,
        ...
        ofs.open(fileName);
        if (! ofs.is_open()) {
            PRINT_RT("Failed to open file %s\n", fileName.c_str());
            return false;
        }
	...
	static bool storeMatrixDistributed(std::string const & fileName, std::vector<std::string> const & row_names,
        ...
        if (row_names.size() == input.rows()) {
            ...
            if (procs == 1) {
                ...
                ROOT_PRINT("ERROR: unpartitioned data %lu but multiple MPI procs %d.\n", input.rows(), procs);
                return false;
            }
        }
        ...
        if (total != row_names.size()) {
            ROOT_PRINT("ERROR: input partitioning incorrect.  total %lu != num of rows %lu\n", total, row_names.size());
            return false;
        }
        ...
        MPI_File_close(&fh);
        // PRINT_RT("MPI_IO remaining %lu bytes, finishing at %lu\n", lsize, offset);
        MPI_Bcast(&offset, 1, MPI_UNSIGNED_LONG, procs - 1, comm);

        etime = getSysTime();
        ROOT_PRINT("MPI-IO wrote %lu bytes in %f sec\n", offset, get_duration_s(stime, etime));
