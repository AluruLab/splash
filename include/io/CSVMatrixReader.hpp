...
class FileReader {
	protected:
		...
		inline char_array open(const char * filename) {
		...
		inline void close(char_array & buffer) {
#ifndef NDEBUG
			int rc = 
#endif
	...
	public:
		...
		FileReader(const char * filename, MPI_Comm comm = MPI_COMM_WORLD) {
			int rank;
			MPI_Comm_rank(comm, &rank);

			if (rank == 0) {  // rank 0 read the file and broadcast.
				data = this->open(filename); 
				mapped = true;
		...
		FileReader(const char * filename) : mapped(true) {
			data = this->open(filename);
		}
#endif
		virtual ~FileReader() {
	...
	public:
		...
		size_t count_token_or_empty(char_array & buffer, const char (&delim)[N]) {
			...
			return std::count_if(buffer.start, buffer.start + buffer.size,
...
const char FileReader::CR = '\r';
const char FileReader::COMMA[] = ",";
const char FileReader::TAB[] = "\t";
...
bool CSVMatrixReader<FloatType>::loadMatrixData_impl(
	...
	for (; (token.start != nullptr) && (numSamples < vectorSize); 
		...
		samples.push_back(std::string(token.start, 0, token.size));
	}
	/*check consistency*/
	...
	for (; (line.start != nullptr)  && (numGenes < numVectors);
		...
		// parse the row name
		token = this->get_token_or_empty(line, FileReader::COMMA);
		genes.push_back(std::string(token.start, 0, token.size));
		...
		for (; (token.start != nullptr) && (numSamples < vectorSize); 
			...
			std::string s(token.start, 0, token.size);
